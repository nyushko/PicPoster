...
public class PicPosterController {
	...
	public PicPosterController(PicPosterModelList model,
			PicPosterActivity activity) {
		this.model = model;
		this.activity = activity;
	}

	/**
	 * Adds the new pic post to the model, adding a default pic and text if none
	 * is assigned, and scaling the pic and shortening text as necessary.
	 * 
	 * @param pic
	 *            the Bitmap to be displayed in the post
	 * @param text
	 *            the String to be displayed in the post
	 */
	public void addPicPost(Bitmap pic, String text) {

		// Assign a default pic if there is none:
		if (pic == null)
			pic = BitmapFactory.decodeResource(this.activity.getResources(),
					R.drawable.no_img);
